...
### 六类：M2 相关（踩坑 & 没注意到的点）
**踩坑（代码）**
- `run` 开头**漏了** `messages.append({"role":"user","content":user_input})` —— 用户输入没进记忆、AI 不知道用户问了啥。
- `__main__` **少了个 `while True:`** —— 没包循环，`break` 在模块级直接 `SyntaxError`。
- `__main__` 拼错 **`user_iput`**（应为 `user_input`）。
- `__main__` 里 `input()`、`break`、`print(run(...))` 都要**缩进到 `while` 里**。
- `"contents"` → **`"content"`**（多了个 s）。

**理解要点（原理）**
- **M2 = 多工具(now) + 多轮对话(`__main__` 循环) + 记忆(messages 提外面)** 三件事。
- **记忆的本质**：函数里的变量=局部（用完就扔，记不住）；放到外面=全局（一直累积，记得住）。
- `__main__` 循环负责"**多轮**"，`messages` 在外面负责"**记忆**"——两者配合才有"有记忆的连续对话"；光有循环没记忆，还是每轮失忆。
- 两个 `while` 是**两层**：① 内层(`run`)=处理一句话的"想→调工具→再想"；② 外层(`__main__`)=一整场对话"问你→跑→再问你"。
